...
Consider the case where a top-up and an offline debit happen at the same time. The starting state is chip \texttt{(balance=50, debit\_counter=0, credit\_counter\_seen=0)} and server \texttt{(balance=50, credit\_counter=0, debit\_counter\_seen=0)}.

The user pays 10 tokens at an offline terminal. The chip becomes \texttt{(40, 1, 0)} and the terminal queues a debit of 10 with 
\texttt{debit\_counter=1}. The server has not heard about this yet.
...
The user then taps an online terminal somewhere else. The terminal reads \texttt{(40, 1, 0)} from the chip and forwards it to the server. The server sees that \texttt{credit\_counter=1} is greater than the chip's 
\texttt{credit\_counter\_seen=0}, so there is one pending credit of 20 tokens to apply. It also sees that \texttt{debit\_counter=1} is greater than  \br
\texttt{debit\_counter\_seen=0}, so there is one chip-side debit it has not received yet, but it does not need the amount to apply the credit safely. The server writes \texttt{(60, 1, 1)} to the chip.
