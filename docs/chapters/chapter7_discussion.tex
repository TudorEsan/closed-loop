...
The reconciliation model is correct as long as terminals eventually sync. If a terminal dies before it can push its queue, the chip subtracted the amount but the server has no record. The gap is detectable from the counter, but the transaction details are gone with the device. It is worth being precise about what kind of loss this is. By the time a debit sits in the queue, the chip was already decremented and the customer walked away with the goods, and the festival already holds that customer's money from the top-up. So a lost queue does not let anyone create money or take money from an attendee; what it destroys is the itemized record that says which vendor made those sales, so the festival cannot settle them from its own books. The loss is an accounting one, bounded by the real sales made at that one terminal during the outage. The design keeps this worst case sizeable in advance: cap the queue length and the per-transaction amount, and the product of the two is the most a single dead terminal can cost. The terminal enforces both limits in the offline charge path (Section~\ref{sec:impl-offline-debit}): it stops taking offline payments once 50 debits are pending and refuses any single offline payment above 100.00, and both checks run before the chip write so a refused payment never touches the bracelet. With these values the worst case per terminal is 5000.00, a number the operator can size to whatever loss the festival is willing to absorb. The limits are constants in the current build; provisioning them per event is left as production work.
...
The reconciliation model assumes that outages are short, on the order of minutes, not hours. The maximum amount that can be lost from a dead terminal scales with how long the terminal has been offline, and during a multi-hour outage a single terminal could buffer hundreds of transactions before its battery dies. The terminal does enforce a hard cap on the queue (it refuses new offline payments once 50 debits are pending, Section~\ref{sec:impl-offline-debit}), so the buffering is bounded. What is missing is an escalation path for prolonged outages: once a terminal hits the cap it simply refuses offline payments, and there is no operator flow for prioritising a sync of that terminal, raising the limit on the spot, or moving the vendor to another device. The cap is also a compile-time constant, so adjusting it per event means rebuilding the app, which a production deployment would replace with per-event provisioning.
